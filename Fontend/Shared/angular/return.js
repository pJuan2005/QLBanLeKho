...
app.controller("returnCtrl", function ($scope, $http, $timeout, AuthService, PermissionService, $window, TranslateService) {
    ...
    $scope.add = function () {
        ...
        if (r.ReturnType === 2 && !r.POID) {
            alert("⚠️ Supplier return cần POID!");
            $scope.savingAdd = false;
            return;
        }
        ...
        const payload = {
            ...
            POID: r.ReturnType === 2 ? r.POID : null,
    ...
    $scope.onTypeChange = function () {
        ...
        if (type === 1) {
            $scope.newReturn.POID = null;   // CUSTOMER
        } else if (type === 2) {
            $scope.newReturn.SaleID = null; // SUPPLIER
        }
    };
    ...
    $scope.updateReturn = function () {
        ...
        const payload = {
            ...
            ProductID: $scope.editingReturn.ProductID,
            Quantity: $scope.editingReturn.Quantity,
            ...
            POID: $scope.editingReturn.ReturnType == 2 ? $scope.editingReturn.POID : null
        };
        ...
        $http.post(current_url + "/api-core/return/update", payload)
            ...
            .catch(err => {
                ...
                // --- xử lý lỗi chi tiết ---
                if (msg.includes("POID bắt buộc")) {
                    alert("⚠️ Khi ReturnType = 2 (Supplier Return), POID là bắt buộc!");
                }
                ...
                else if (msg.includes("POID không tồn tại")) {
                    alert("⚠️ POID không tồn tại trong hệ thống!");
                }
                else {
                    alert(msg);
                }
